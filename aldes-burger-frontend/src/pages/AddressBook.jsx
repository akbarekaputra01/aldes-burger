...
import { loadGoogleMapsPlaces, reverseGeocode, searchAddressSuggestions } from './addressbook/geocoding'
...
function AddressBook() {
  ...
  const mapRef = useRef(null); const mapElRef = useRef(null); const markerRef = useRef(null); const streetInputRef = useRef(null); const placeAutocompleteRef = useRef(null)
  ...
  useEffect(() => {
    let mounted = true
    loadGoogleMapsPlaces().then((google) => {
      if (!mounted || !google?.maps?.places || !streetInputRef.current || placeAutocompleteRef.current) return
      ...
      autocomplete.addListener('place_changed', () => {
        ...
        for (const component of components) {
          ...
          if (type === 'administrative_area_level_1') province = component.long_name?.toUpperCase() || ''
          if (type === 'administrative_area_level_2') city = component.long_name?.toUpperCase() || ''
          if (type === 'administrative_area_level_3' || type === 'sublocality_level_1') district = component.long_name?.toUpperCase() || ''
        ...
        setForm((prev) => ({
          ...prev,
          street_address: place.formatted_address || place.name || prev.street_address,
          province: province || prev.province,
          city: city || prev.city,
          district: district || prev.district,
          postal_code: postalCode || prev.postal_code,
          latitude: Number.isFinite(lat) ? Number(lat.toFixed(8)) : prev.latitude,
          longitude: Number.isFinite(lng) ? Number(lng.toFixed(8)) : prev.longitude,
          pin_source: Number.isFinite(lat) && Number.isFinite(lng) ? 'suggestion' : prev.pin_source,
          pin_confirmed: Number.isFinite(lat) && Number.isFinite(lng) ? true : prev.pin_confirmed,
        }))
      })
    }).catch(() => {})
  ...
  {form.district && postalCodeOptions.length === 0 && <p className="text-sm text-gray-500">Rekomendasi postal code belum tersedia, silakan ketik manual 5 digit.</p>}
