...
import { getCityOptions, getDistrictOptions, getPostalCodeOptions, getProvinceOptions } from './addressbook/regions'
...
function AddressBook() {
  ...
  const cityOptions = useMemo(() => getCityOptions(form.province), [form.province])
  const districtOptions = useMemo(() => getDistrictOptions(form.province, form.city), [form.province, form.city])
  const postalCodeOptions = useMemo(() => getPostalCodeOptions(form.province, form.city, form.district), [form.province, form.city, form.district])
  ...
  {suggestions.length>0 && <div className="rounded-xl border p-2">{suggestions.map((s)=><button type="button" key={s.id} className="block w-full text-left p-2 hover:bg-gray-50" onClick={()=>setForm((p)=>({ ...p, street_address: s.formattedAddress || s.title, latitude: s.latitude, longitude: s.longitude, pin_source:'suggestion', pin_confirmed:true }))}><div className="font-semibold">{s.title}</div><div className="text-sm text-gray-500">{s.formattedAddress}</div></button>)}</div>}
