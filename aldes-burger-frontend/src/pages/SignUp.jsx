...
function SignUp() {
  ...
  return (
    <main className="min-h-screen w-full overflow-hidden bg-aldesCream selection:bg-aldesYellow selection:text-black relative">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4 py-4 md:px-8 md:py-5 perspective-wrapper">
        <div className={`signup-flip-card ${isFlippingOut ? 'signup-flip-out' : ''} w-full max-w-5xl min-h-[520px] grid grid-cols-1 lg:grid-cols-[0.92fr_1.08fr] bg-white/80 backdrop-blur-xl border-[4px] border-black rounded-[2rem] md:rounded-[2.6rem] overflow-hidden shadow-[10px_10px_0_0_#000]`}>
          ...
          <section className="relative bg-aldesRed px-7 py-7 md:px-9 md:py-8 flex flex-col justify-between overflow-hidden">
            ...
            <div className="relative z-10">
              <Link to="/" title="Back to Home" className="inline-flex items-center gap-2 bg-black text-aldesYellow px-4 py-2 rounded-full border-2 border-aldesYellow font-black text-xs uppercase tracking-wider shadow-[4px_4px_0_0_#FFC926] hover:scale-105 hover:bg-gray-900 active:scale-95 transition-all cursor-pointer">
            ...
            <div className="w-full max-w-[520px] mx-auto">
              ...
              {error && (
                <div className="mb-5 bg-aldesRed/10 border-[3px] border-aldesRed p-3.5 rounded-2xl flex items-start gap-3">
                  <div className="bg-aldesRed text-white rounded-full p-1 shrink-0"><AlertCircle size={17} /></div>
              ...
              {success && (
                <div className="mb-5 bg-green-100 border-[3px] border-green-600 p-3.5 rounded-2xl flex items-start gap-3">
                  <div className="bg-green-600 text-white rounded-full p-1 shrink-0"><CheckCircle size={17} /></div>
              ...
              {step === 'register' ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="group">
                      ...
                      <div className="flex items-center bg-aldesCream/60 border-[3px] border-black rounded-2xl px-4 py-3 focus-within:bg-white focus-within:shadow-[5px_5px_0_0_#FFC926] transition-all">
                        ...
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="ml-2 text-black hover:text-aldesRed transition-colors shrink-0">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                      </div>
                    </div>
                    <div className="group">
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-6 mt-4">
                  ...
                  <div className="text-center">
                    <button type="button" onClick={() => setStep('register')} className="text-xs font-bold text-gray-500 hover:text-aldesRed underline underline-offset-4">Wrong email? Go back</button>
                  </div>
                </form>
              )}
      ...
      <style>{`
        ...
        .signup-flip-card {
          ...
          animation: signupFlipIn 0.85s cubic-bezier(0.16, 1, 0.3, 1) both; will-change: transform, opacity, filter;
        }
        .signup-flip-card.signup-flip-out {
          pointer-events: none; animation: signupFlipOut 0.75s cubic-bezier(0.16, 1, 0.3, 1) both;
        }
        ...
        @keyframes signupFlipOut {
          0% { opacity: 1; filter: blur(0); transform: rotateY(0deg) translateY(0) scale(1); }
          45% { opacity: 0.92; filter: blur(0.5px); transform: rotateY(-4deg) translateY(2px) scale(0.995); }
          100% { opacity: 0; filter: blur(4px); transform: rotateY(-10deg) translateY(18px) scale(0.975); }
        }
        @media (prefers-reduced-motion: reduce) { .signup-flip-card, .signup-flip-card.signup-flip-out { animation: none; filter: none; } }
      `}</style>
    </main>
  );
}
